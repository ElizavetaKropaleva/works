...
def loading_registers(key):
    ...
    for i in range(reg_r_length):
        reg_r.insert(i, int(key[i + reg_z_length + reg_y_length + reg_x_length]))
...
def get_keystream(length):
    ...
    i = 0
    while i < length:
        majority = get_majority(reg_r_temp[3], reg_r_temp[7], reg_r_temp[10])
        if reg_r_temp[10] == majority:
            new = reg_x_temp[12] ^ reg_x_temp[14] ^ reg_x_temp[15]
        ...
        if reg_r_temp[3] == majority:
            new_one = reg_y_temp[9] ^ reg_y_temp[13] ^ reg_y_temp[16]
        ...
        if reg_z_temp[7] == majority:
            new_two = reg_z_temp[13] ^ reg_z_temp[16] ^ reg_z_temp[18]
        ...
        keystream.insert(i, reg_r_temp[11] ^ reg_r_temp[16])
        i = i + 1
    return keystream
...
# Ввод ключа
def input_key():
    tha_key = str(input('Введите ключ (81 бит): '))
...
def main():
    ...
    result_test = ''  # ожидаемый результат шифрования
    ...
    if ch == 1:
        ...
        key = str(input_key())  # Используется ключ: 010100100001101011000111000110010010100100000011011111101011011100111010011011100
        set_key(key)
