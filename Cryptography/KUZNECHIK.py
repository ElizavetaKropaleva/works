...
test = '1122334455667700ffeeddccbbaa9988' # открытый текст, используемый для тестирования
result_test = '7f679d90bebc24305a468d42b9d4edcd' # ожидаемый результат шифрования

# Новый класс с основными функциями
class gost2015:
    def __init__(self, key):
        ...
        f = open(dirname(__file__) + '/gost', 'rb')
    ...
    # Х-элемент из pi таблицы
    def pitransformation(self, x):
        return self.pi[x]
    # Х-элемента из инверсионной pi таблицы
    def piinvtransformation(self, x):
        return self.piinv[x]
    # Замена по таблице pi
    def stransformation(self, x):
        return [self.pitransformation(x[i]) for i in range(len(x))]
    # Инверсионный S процесс
    def sinvtransformation(self, x):
        return [self.piinvtransformation(i) for i in x]
    # Работа с полем x^8 + x^7 + x^6 + x + 1
    def add_field(self, x, y): # Дополнение
        return x ^ y
    def sum_field(self, x): # Сумма элементов
        s = 0
        for a in x:
            s ^= a
        return s
    def mult_field(self, x, y): # Умножение 
        p = 0
        while x:
            if x & 1:
                p ^= y
            ...
            x >>= 1
        return p
    # Преобразование списка байтов в один байт с использованием арифметики конечного поля
    def l(self, x):
    ...
    def linvtransformation(self, x):
        ...
        return x
    # F преобразование, используемое для ключевого расписания
...
def encryption_gost():
    text = input('Шифрование\nВведите текст: ') # 1122334455667700ffeeddccbbaa9988
    mtest = list(binascii.unhexlify(text))
...
def decryption_gost():
    text = input('Расшифрование\nВведите текст: ') # 1122334455667700ffeeddccbbaa9988
    mtest = list(binascii.unhexlify('1122334455667700ffeeddccbbaa9988'))
    ktest = list(binascii.unhexlify('8899aabbccddeeff0011223344556677fedcba98765432100123456789abcdef'))
    gost = gost2015(ktest)
    c = gost.encryption(mtest)
    d = gost.decryption(c)
    return binascii.hexlify(bytearray(d))


def main():
    # Процесс шифрования и вывод результата
    result_encryption = encryption_gost()
    print('Результат шифрования:', str(result_encryption)[2:-1])

    # Проверка совпадения с ожидаемым результатом
    check = bool(result_test == str(result_encryption)[2:-1])
    print('Результат проверки:', check, '\n')

    # Процесс расшифрования и вывод результата
    result_decryption = decryption_gost()
    print('Результат расшифрования:', str(result_decryption)[2:-1])

    # Проверка совпадения с ожидаемым результатом
    check = bool(test == str(result_decryption)[2:-1])
    print('Результат проверки:', check, '\n')
