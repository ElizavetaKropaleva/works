...
def encryption(text, alphabet, key, action):
    n = len(alphabet)
    result = ''
    text_key = key
    z = -1
    if (action == 'encode'):
        z = 1
        text_key += text  # добавление буквы-ключа к тексту
    for i in range(len(text)):
        sym = alphabet[(alphabet.index(text[i]) + z * alphabet.index(text_key[i])) % n]
        result += sym
        if (action == 'decode'):
            text_key += sym
    return result
...
def main():
    ...
    result_test = 'ттжпчърряпбктйсхъаукрчрцбтбпчърлъщвчлукрчыйб'  # ожидаемый результат шифрования
    ...
    if ch == 1:
        ...
        key = input('Ключ = ')  # буква-ключ шифрования при тестирования (shift = е)

        # Процесс шифрования и вывод результата
        result_encryption = encryption(test, alphabet_test, key, 'encode')
        ...
        result_decryption = encryption(result_encryption, alphabet_test, key, 'decode')
        ...
        key = input('Ключ = ')  # буква-ключ шифрования

        # Преобразование текста к нижнему регистру
        text = text.lower()

        # Процесс шифрования и вывод результата
        result_encryption = encryption(text, alphabet_text, key, 'encode')
        print('Результат шифрования:', result_encryption)

        # Процесс расшифрования и вывод результата
        result_decryption = encryption(result_encryption, alphabet_text, key, 'decode')
