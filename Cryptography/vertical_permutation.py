...
def encryption(text, key, action):
    ...
    if action == 'encode':
        ...
        index = 0
        z = len(text) % len(key)
        arr = list(range(0, len(text)))
        for i in key_int:
            k = len(text) // len(key)
            if i < z:
                k += 1
            for j in range(0, k):
                arr[j * len(key) + i] = text[index]
                index += 1
        result = ''.join(arr)
...
def main():
    ...
    result_test = 'йкиаооеоезслкнсрййачсивблйббхпяиябоочтоалтмв'  # ожидаемый результат шифрования
    ...
    if ch == 1:
        ...
        key = input('Ключ = ')  # ключ шифрования при тестирования (shift = октябрь)

        # Процесс шифрования и вывод результата
        result_encryption = encryption(test, key, 'encode')
        ...
        result_decryption = encryption(result_encryption, key, 'decode')
        ...
        text = text.lower()
        key = input('Ключ = ')  # ключ шифрования при тестирования

        # Процесс шифрования и вывод результата
        result_encryption = encryption(text, key, 'encode')
        print('Результат шифрования:', result_encryption)
