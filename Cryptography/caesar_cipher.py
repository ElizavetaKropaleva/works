...
def cesar(text, alphabet, shift, action):  # action - определяет, какое действие будет выполняться (шифрование/расшифрование)
    length = len(alphabet)
    result = ''
    if action == 'decode':
        shift = length - shift
    for i in text:
        result += alphabet[(alphabet.index(i) + shift) % length]
    return result
...
def main():
    ...
    result_test = 'ридсмфвфсдгнлдуишолесмктхгдсмфвпсоъголесмхън'  # ожидаемый результат шифрования
    ...
    if ch == 1:
        ...
        shift = int(input('Ключ = '))  # ключ шифрования при тестирования (shift = 3)

        # Процесс шифрования и вывод результата
        result_encryption = cesar(test, alphabet_test, shift, 'encode')
        print('\nРезультат шифрования:', result_encryption)
        ...
        result_decryption = cesar(result_encryption, alphabet_test, shift, 'decode')
        print('Результат расшифрования:', result_decryption)

        # Проверка совпадения с первоначальным текстом
        ...
        shift = int(input('Ключ = '))  # ключ шифрования

        # Преобразование текста к нижнему регистру
        text = text.lower()
        # Процесс шифрования и вывод результата
        result_encryption = cesar(text, alphabet_text, shift, 'encode')
        print('Результат шифрования:', result_encryption)

        # Процесс расшифрования и вывод результата
        result_decryption = cesar(result_encryption, alphabet_text, shift, 'decode')
