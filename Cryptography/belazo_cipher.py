...
def encryption(text, alphabet, key, action):
    ...
    for i in range(0, len(text), len(key)):
        for y in range(len(key)):
            if (i + y < len(text)):
                result += alphabet[(alphabet.index(text[i + y]) + z * alphabet.index(key[y])) % n]
    return result
...
def main():
    ...
    result_test = 'яулачысяшуофъпъчгхършыхщдолачысъшэекэцмачьйш' # ожидаемый результат шифрования
    ...
    if ch == 1:
        ...
        key = input('Ключ = ') # ключ шифрования при тестирования (shift = ток)
        ...
        print('Текст:', text, '\n')
        key = input('Ключ = ') # ключ шифрования
